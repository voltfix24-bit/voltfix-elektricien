-- PREPARED ONLY. Requires the applied Ads migrations from 2026-09-20 to 2026-09-22.
-- Includes browser-binding columns; the separate version-D pending SQL is not required.
-- No legacy grants are promoted into this new, browser-bound consent ledger.
BEGIN;
...
BEGIN
  ...
  END IF;
  -- Every decision and every export claim locks this same row first.
...
-- New records cannot recreate a withdrawn grant from a delayed form request.
-- INSERT only: unrelated backoffice updates retain their existing behaviour.
...
BEGIN
  -- First read locates the subject only. Re-read the lead after acquiring its lock.
  ...
  END IF;
  -- The history policy is also evaluated here, not only in a prior HTTP read.
...
-- Protect immutable payloads even if an old application instance is still running.
