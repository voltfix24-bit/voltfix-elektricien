...
export function perilexFaqs(lang: GroupLocale): { q: string; a: string }[] {
  ...
  if (lang === 'en') {
    return [
      ...
      { q: 'How quickly can you come out?', a: `Often the same week, sometimes the same day, depending on availability. ${priority} is the total rate for the same straightforward job with priority within 24 hours, and only after we have confirmed availability. It is not a surcharge on top of the standard rate.` },
  ...
  return [
    ...
    { q: 'Hoe snel kunnen jullie komen?', a: `Vaak dezelfde week, soms dezelfde dag, afhankelijk van beschikbaarheid. ${priority} is het totale tarief voor dezelfde eenvoudige klus met voorrang binnen 24 uur, en alleen na bevestigde beschikbaarheid. Het is dus geen toeslag bovenop het standaardtarief.` },
